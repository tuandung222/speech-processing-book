---
title: "End-to-End TTS & Zero-Shot Voice Cloning"
---

## 7.1 Từ Two-Stage đến End-to-End
...
## 7.2 VITS — Variational Inference with Adversarial Learning
...
```
                    TRAINING
Text ─→ [Text Encoder] ─→ h_text ─→ [Duration Predictor]
              │                              │
              │         ┌────────────────────┘
              ▼         ▼
     [Stochastic Duration]    Audio ─→ [Posterior Encoder] ─→ z_audio
              │                              │
              ▼                              ▼
     Prior (text-conditioned)          [Normalizing Flow]
         p_θ(z|c)          ←──align──→    q_φ(z|x)
                                             │
                                             ▼
                                      [HiFi-GAN Decoder]
                                             │
...
### 7.2.5 Monotonic Alignment Search (MAS)
...
## 7.3 VALL-E — Neural Codec Language Model for TTS

### 7.3.1 Paradigm Shift
...
::: {.callout-tip title="NLP Parallel: GPT for Speech"}
...
Stage 1: Autoregressive (AR) Model
    ...
    Input:  [text; prompt_c1] → predict c1_t autoregressively
    Output: c^(1) = [c^(1)_1, c^(1)_2, ..., c^(1)_T]

Stage 2: Non-Autoregressive (NAR) Model
    Predict codebooks 2-8 in parallel, conditioned on previous
    Input:  [text; c^(1); c^(2); ...; c^(q-1)] → predict c^(q) all at once
    Output: c^(2), c^(3), ..., c^(8)

Finally: EnCodec decoder converts all 8 codebooks → waveform
```

### 7.3.3 AR Model (Codebook 1)
...
: VALL-E comparison {#tbl-valle-comparison}

## 7.4 F5-TTS — Flow Matching + DiT
...
$$ {#eq-flow-interpolation}

::: {.callout-note title="Flow Matching vs Diffusion"}
...
### 7.4.2 DiT Architecture (Diffusion Transformer)

F5-TTS sử dụng **DiT** (Diffusion Transformer) thay vì U-Net:

```
Input: noisy mel x_t + text embedding + time embedding
       │
       ▼
┌─────────────────────────────┐
│  DiT Block (× N)             │
...
: End-to-end TTS comparison {#tbl-e2e-tts}

## 7.5 TTS Evolution Timeline
...
: TTS evolution timeline {#tbl-tts-timeline}
...
: End-to-end TTS summary {#tbl-e2e-summary}
