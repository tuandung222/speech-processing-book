...
1. **Underscore trong `\text{}`**: tránh dùng underscore bên trong `\text{}`. Viết `\text{frame length}` cho nhãn văn xuôi, hoặc dùng `\mathrm{frame\_length}` nếu cần giữ tên biến dạng snake case.
