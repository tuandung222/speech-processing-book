...
- Benchmark nào được dùng để so sánh model? Những kết quả công khai nên được đọc trong bối cảnh nào?
...
Bảng dưới đây không nên được đọc như một leaderboard tuyệt đối. Các hệ thống thương mại thường không công bố đầy đủ test set, protocol đánh giá và điều kiện nhiễu; vì vậy các con số WER dạng `~` chỉ có giá trị tham khảo định hướng. Khi ra quyết định kỹ thuật, cần tái đánh giá trên tập dữ liệu nội bộ có cùng domain, phương ngữ và điều kiện thu âm với sản phẩm.

| Model | Tổ chức | Base | WER/VLSP công khai | Open-source |
|-------|---------|------|--------------------|-------------|
| PhoWhisper | VinAI | Whisper | Khoảng 7-11% tuỳ split và paper | Có (HuggingFace) |
| Wav2Vec2-vi | VinAI/community | Wav2Vec 2.0 | Khoảng 10% trên một số benchmark công khai | Có |
| XLSR-Vietnamese | Community | XLSR-53 | Khoảng 12% trong báo cáo cộng đồng | Có |
| Viettel ASR | Viettel | Conformer/production stack | Không có protocol công khai đầy đủ | Không (API) |
| Zalo ASR | Zalo/VNG | Production ASR | Không có protocol công khai đầy đủ | Không (API) |
| FPT.AI ASR | FPT | Production ASR | Không có protocol công khai đầy đủ | Không (API) |
| NVIDIA Canary-vi | NVIDIA | Canary | Chưa có kết quả VLSP công khai ổn định | Có (NeMo) |
...
Với TTS tiếng Việt, MOS công khai thường khó so sánh hơn WER vì phụ thuộc panel đánh giá, thiết bị nghe, transcript, phương ngữ và tiêu chí naturalness. Do đó, bảng này chỉ dùng để định vị hệ sinh thái, không thay thế cho human evaluation nội bộ.

| Model | Tổ chức | Kiến trúc | Chất lượng công khai | Open-source |
|-------|---------|----------|----------------------|-------------|
| VietTTS | Community | VITS | Baseline cộng đồng, phù hợp thử nghiệm | Có |
| Zalo TTS | Zalo/VNG | Production TTS | Không có MOS/protocol công khai đầy đủ | Không (API) |
| FPT.AI TTS | FPT | Production TTS | Không có MOS/protocol công khai đầy đủ | Không (API) |
| Viettel TTS | Viettel | Production TTS | Không có MOS/protocol công khai đầy đủ | Không (API) |
| CosyVoice-vi | Alibaba/community fine-tune | CosyVoice | Chưa có MOS tiếng Việt công khai đáng tin cậy | Có thể fine-tune |
...
2. **PhoWhisper** (VinAI) là một baseline open-source mạnh cho Vietnamese ASR, nhưng cần đọc kết quả theo đúng benchmark và thời điểm công bố
