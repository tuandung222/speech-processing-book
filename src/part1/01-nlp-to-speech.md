...
>
> Mỗi khi gặp một khái niệm Speech mới, mình sẽ luôn dừng lại và bắc cầu sang NLP equivalent. Bạn sẽ thấy rằng "thư viện trực giác" bạn đã xây dựng nhiều năm qua không hề bị vứt bỏ. Nó chỉ cần được mở rộng.
...
Trick nhỏ: speech LM thường có nhiều codebook layers (Moshi có 8). KV cache cho mỗi codebook tách biệt hoặc chia sẻ? Câu trả lời: Moshi dùng "depth transformer" — một transformer nhỏ xử lý codebook layers độc lập với KV cache riêng. Đây là một kỹ thuật quan trọng sẽ giải thích ở Chương 11.
...
Speech dataset nhỏ hơn nhưng mỗi sample "đắt" hơn để process (Mel preprocessing, longer sequences). Tổng compute training Whisper-Large-v3 và LLaMA-3 8B tương đương — khoảng 1-2 triệu H100 GPU-hours.
...
>
> Nếu bạn đang debug performance của Speech model và thấy chậm hơn LM cùng size, đừng ngạc nhiên — đó là consequence trực tiếp của data rate gap. Optimization như flash attention, KV cache compression, codec ở fps thấp đều quan trọng hơn ở Speech so với NLP.
...
Streaming TTS state-of-the-art năm 2026 (Cartesia Sonic, ElevenLabs Turbo) đạt latency ~200 ms với cost ~$0.0001 per character. Nghe thấp nhưng cho một call center handle 1 triệu conversations, chi phí lên tới hàng nghìn USD/ngày.

Compared với text LLM ($0.0001 per token, generate vài trăm token per response), audio interaction đắt hơn ~10-100x. Đây là rào cản kinh tế cho widespread deployment.

Mimi 1.1 kbps + Moshi giúp giảm đáng kể (toàn bộ codec inference cho 1 giây cost ~$0.00001). Nhưng vẫn cần improvements để cạnh tranh với text-only chatbots về cost.
...
- Đi tuần tự Chương 1 → 20.
- Mỗi chương spend 2-3 giờ đọc kỹ + chạy code.
- Total time: ~60-80 giờ cho exposure đầy đủ.
...
Đây là chương nặng về toán/DSP nhất của cuốn sách. Nếu bạn từ NLP, bạn có thể không quen với spectral analysis. Đừng lo — mình sẽ giải thích từ ground up, với analogy NLP/CV mỗi khi có thể.
