...
Chương này phân tích các kiến trúc tiêu biểu theo dòng thời gian: Qwen2.5-Omni (Thinker-Talker pattern), GPT-Realtime (OpenAI, GA tháng 8/2025), Gemini Live (Google), Qwen3-Omni (Alibaba, MoE Thinker-Talker, công bố tháng 9/2025), và các báo cáo frontier tiếp theo trong năm 2026.
...
Multimodal Omni models là một xu hướng lớn trong AI 2024-2026: một model duy nhất xử lý **text, audio, image, video** đồng thời. Chương này phân tích các kiến trúc tiêu biểu: **Qwen2.5-Omni**, **GPT-Realtime**, **Gemini Live**, **Qwen3-Omni**, và các frontier reports tiếp theo.
...
4. **Thinker-Talker**: tách reasoning/planning khỏi speech generation (Qwen2.5-Omni, Qwen3-Omni)
...
Qwen3-Omni là một mốc quan trọng của open-source Speech LLM. Theo README và technical report chính thức của QwenLM, đây là họ model **natively end-to-end multilingual omni-modal**, xử lý text, image, audio và video, đồng thời có thể trả lời bằng text hoặc natural speech. Kiến trúc trung tâm là **MoE Thinker-Talker**: Thinker đảm nhiệm understanding và reasoning đa phương thức, còn Talker sinh speech output theo streaming.

### Variants

- **Qwen3-Omni-30B-A3B-Instruct**: text, image, audio, video input; text và audio output. Tổng 30B parameters với khoảng 3B active parameters per forward pass (MoE).
- **Qwen3-Omni-30B-A3B-Thinking**: tập trung vào reasoning trên multimodal input, output chính là text.
- **Qwen3-Omni-30B-A3B-Captioner**: chuyên cho audio captioning chi tiết, giảm hallucination trong mô tả audio.

### Capabilities

- 119 ngôn ngữ text input.
- 19 ngôn ngữ speech input, gồm English, Chinese, Korean, Japanese, German, Russian, Italian, French, Spanish, Portuguese, Malay, Dutch, Indonesian, Turkish, **Vietnamese**, Cantonese, Arabic, Urdu.
- 10 ngôn ngữ speech output: English, Chinese, French, German, Russian, Italian, Spanish, Portuguese, Japanese, Korean. **Vietnamese chưa nằm trong danh sách speech output chính thức** ở README QwenLM/Qwen3-Omni.
- Apache 2.0 license, open weights trên Hugging Face/ModelScope.

### Performance (theo Qwen3-Omni Technical Report và README chính thức, tháng 9/2025)
...
- ASR, audio understanding và voice conversation được báo cáo là cạnh tranh với Gemini 2.5 Pro trên nhiều benchmark; khi dùng production vẫn cần benchmark lại theo ngôn ngữ, domain và latency budget riêng.

### Qwen3-Omni-Flash (December 2025)

Variant optimised cho inference latency, hướng tới production voice agent.

### Ghi chú deployment và serving

Qwen3-Omni có thể chạy qua Transformers hoặc vLLM-Omni. Với vLLM-Omni, recipe chính thức dùng `vllm serve Qwen/Qwen3-Omni-30B-A3B-Instruct --omni`, có thể chạy single-process hoặc tách stage: **Stage 0 Thinker + API server**, **Stage 1 Talker**, **Stage 2 Code2Wav**. Thiết kế stage-based này phản ánh đúng bản chất production của Omni speech: reasoning, speech token generation và codec-to-waveform có profile latency/memory khác nhau.
...
- Theo công bố chính thức, Qwen3.5-Omni Plus so sánh thuận lợi với Gemini 3.1 Pro trên một số benchmark general audio understanding, reasoning và translation. Cần đọc kết quả này theo đúng protocol benchmark, thời điểm công bố và workload triển khai.
...
| Qwen3-Omni 30B-A3B | Alibaba | Sep 2025 | Có | Có | Có | Có |
...
4. **Qwen3-Omni** là nhóm open-source frontier đáng chú ý ở thời điểm viết; với tiếng Việt, điểm quan trọng là có Vietnamese speech input nhưng chưa có Vietnamese speech output chính thức, nên voice agent Việt vẫn thường cần TTS tiếng Việt riêng.
